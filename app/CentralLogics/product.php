...
class ProductLogic
{
    ...
    {
        $paginator = Product::active()->withCount(['wishlist'])->with(['rating'])->where('discount', '=', 0.00)->where('no_to_purchase', '=', 0)->latest()->paginate($limit, ['*'], 'page', $offset);
    ...
    {
       $mainCategory = MainCategory::where('name', $name)->orwhere('name', 'like', "%{$name}%")->first();
       return dd('["'.$mainCategory->id.'"]');
       if($mainCategory != null){
        $key = explode(' ', $name);
        $paginator = Product::active()->withCount(['wishlist'])->with(['rating'])->where('main_category', '["'.$mainCategory->id.'"]')->paginate($limit, ['*'], 'page', $offset);
