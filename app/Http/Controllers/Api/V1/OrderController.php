            ...
            try {
                ...
                foreach ($request['cart'] as $c) {
                    ...
                    if (count(json_decode($product['variations'], true)) > 0) {
                        ...
                        Product::where(['id' => $product['id']])->update([
                            'variations' => json_encode($var_store),
                            'no_of_purchases' => $product['no_of_purchases'] + $c['quantity']
                        ]);
                    } else {
                        // Product::where(['id' => $product['id']])->update([
                        //     'total_stock' => $product['total_stock'] - $c['quantity']
    ...
    public function get_order_details(Request $request)
    {
        ...
        if ($details->count() > 0) {
            foreach ($details as $det) {
                ...
                $det['product_details'] = Helpers::product_data_formatting(Product::where('id', json_decode($det['product_details'], true)['id'])->first());
            }
