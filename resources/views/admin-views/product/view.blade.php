            ...
            <div class="row">
                ...
                <div class="col-6">
                    <a href="{{url()->previous()}}" class="btn btn-primary float-right">
                        <i class="tio-back-ui"></i> {{trans('messages.back')}}
                    </a>
                </div>
            </div>
            <!-- Nav -->
            ...
            <div class="card-body">
                <div class="row align-items-md-center gx-md-5">
                    ...
                    <div class="col-4 pt-2">
                        ...
                        <label class="badge badge-soft-success">{{trans('messages.total')}} {{trans('messages.stock')}}
                            : {{$product['total_stock']}}</label><br>
                        <span>{{trans('messages.price')}} :
                            <span>{{$product['price']}} {{\App\CentralLogics\Helpers::currency_symbol()}} / {{trans('messages.'.$product['unit'])}}</span>
                        </span><br>
                        <span>{{trans('messages.tax')}} :
                            <span>{{\App\CentralLogics\Helpers::tax_calculate($product,$product['price'])}} {{\App\CentralLogics\Helpers::currency_symbol()}}</span>
                        </span><br>
                        <span>{{trans('messages.discount')}} :
                            <span>{{\App\CentralLogics\Helpers::discount_calculate($product,$product['price'])}} {{\App\CentralLogics\Helpers::currency_symbol()}}</span>
                        </span><br>
                        <h4 class="border-bottom mt-2"> {{trans('messages.variations')}} </h4>
                        @foreach(json_decode($product['variations'],true) as $variation)
                            <span class="text-capitalize">
                              {{$variation['type']}} : {{$variation['price']}} {{\App\CentralLogics\Helpers::currency_symbol()}} ( Stock : {{$variation['stock']??0}} )
                            </span><br>
                        @endforeach
                    </div>

                    <div class="col-8 pt-2 border-left">
                        <h4>{{trans('messages.short')}} {{trans('messages.description')}} : </h4>
